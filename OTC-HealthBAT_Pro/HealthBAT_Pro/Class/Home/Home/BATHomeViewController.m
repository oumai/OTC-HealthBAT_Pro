...
#import "BATHomeMallNewCell.h" //健康商城单个图片Cell
...
#import "BATHomeMallNewViewController.h" //健康商城
...
@interface BATHomeViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        topCircleCell.circlePictureView.placeholderImage = [UIImage imageNamed:@"默认图"];
    ...
    if (indexPath.section == 3) {
        //商城cell
        BATHomeMallNewCell *mallCell = [tableView dequeueReusableCellWithIdentifier:MALL_CELL forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 3) {
        ...
        BATHomeMallNewViewController *homeMallVC = [BATHomeMallNewViewController new];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == 6) {
        ...
        [header.leftButton setTitle:@"健康评测" forState:UIControlStateNormal];
        [header setRightTap:^{
    ...
    if (section == 7) {
        header.rightButton.hidden = YES;
        [header.leftButton setTitle:@"健康计步" forState:UIControlStateNormal];
    }
...
- (UITableView *)homeTableView {
    ...
    if (!_homeTableView) {
      ...
      //  [_homeTableView registerClass:[BATHomeNewMallTableViewCell class] forCellReuseIdentifier:MALL_CELL];
        ...
        [_homeTableView registerNib:[UINib nibWithNibName:@"BATHomeMallNewCell" bundle:nil] forCellReuseIdentifier:MALL_CELL];
...
- (BATKangDoctorView *)kangDoctorView {
    ...
    if (!_kangDoctorView) {
        _kangDoctorView = [[BATKangDoctorView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH-102/2.0, SCREEN_WIDTH*42.0/75.0-212/2.0, 102/2.0, 212/2.0)];
        WEAK_SELF(self);
...
//        _backRoundGuideFloatingView.hidden = NO;
        _backRoundGuideFloatingView.hidden = YES;
