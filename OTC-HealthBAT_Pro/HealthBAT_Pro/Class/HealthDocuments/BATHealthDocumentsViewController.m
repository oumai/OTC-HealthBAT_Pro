...
//    url = [NSString stringWithFormat:@"http://mobile.hmtest.kmhealthcloud.cn:8165?appkey=%@&timestamp=%@&sign=%@&phone=%@&src=2",appkey,timestamp,sign,phone];
    ...
    url = [NSString stringWithFormat:@"http://hc003tn-mobile.chinacloudsites.cn?appkey=%@&timestamp=%@&sign=%@&phone=%@&src=2",appkey,timestamp,sign,phone];
#endif
...
- (UIImageView *)animateView
{
    if (_animateView == nil) {
        ...
        [_animateView setImage:[UIImage imageNamed:@"图层 1"]];
        [_animateView sizeToFit];
        
        NSMutableArray *animates = [NSMutableArray array];
        
        for (int i = 1; i < 21; i++) {
            UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"图层 %ld",(long)i]];
