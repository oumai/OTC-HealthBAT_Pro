...
- (void)layout{
    ...
    [self.consultLable mas_makeConstraints:^(MASConstraintMaker *make) {
        STRONG_SELF(self);
        make.top.equalTo(self.mas_top).offset(41 - _commentIntervalHeight);
...
- (UIView *)bgView{
    if (!_bgView) {
        _bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 10, SCREEN_WIDTH - 90, _commentBigHeight - 20)];
...
- (UIImageView *)doctorIconView{
    if (!_doctorIconView) {
        ...
        _doctorIconView.layer.cornerRadius = 37.5f;
