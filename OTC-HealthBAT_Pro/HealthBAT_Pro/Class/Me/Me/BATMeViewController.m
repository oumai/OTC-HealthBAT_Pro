...
#import "BATMyOrderViewController.h"//我的订单
#import "BATMyPrescriptionVC.h"     //我的处方单
...
- (void)viewDidLoad {
    ...
    _dataSource = [NSMutableArray arrayWithObjects:
                   ...
                   @[
                     @{@"title":@"我的订单",@"image":@"icon-wddd"},
                     @{@"title":@"我的处方单",@"image":@"icon-wdcfd"},
                     @{@"title":@"我的收货地址",@"image":@"icon-shdz"},
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0){
        if (indexPath.row == 0) {
            ...
            //新个人资料，暂时不启用
            //              BATPersonalInfoController *personInfoVC = [[BATPersonalInfoController alloc]init];
            //隐藏tabBar
            ...
            //健康档案
            if (CANCONSULT) {
                //                BATFileListController *treatmentCtl = [[BATFileListController alloc]init];
    ...
    else if (indexPath.section == 1)
    {
        ...
        if (indexPath.row == 0) {
            BATMyOrderViewController *promoCodeVC = [[BATMyOrderViewController alloc]init];
            promoCodeVC.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:promoCodeVC animated:YES];
        }
        else if (indexPath.row == 1)
        {
            BATMyPrescriptionVC *promoCodeVC = [[BATMyPrescriptionVC alloc]init];
            promoCodeVC.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:promoCodeVC animated:YES];
        }
        else if (indexPath.row == 2)
...
//    }
//
////    [TalkingData trackEvent:@"个人中心>关注" label:@"我的好友"];
//
...
//    }
//
////    [TalkingData trackEvent:@"个人中心>粉丝" label:@"我的粉丝"];
//
...
- (void)goMessageCenterAction{
    ...
    BATMessageViewController * messageVC = [[BATMessageViewController alloc] init];
...
{
    //    //个人信息
    //    [TalkingData trackEvent:@"个人中心>个人信息" label:@"个人信息"];
    BATPersonInfoViewController *personInfoVC = [[BATPersonInfoViewController alloc]init];
    //隐藏tabBar
    personInfoVC.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:personInfoVC animated:YES];
