...
- (void)setupUI{
    ...
    if ([self.loginUserID isEqualToString:self.accountID]) {
        ...
        //消息列表控制器
        BATMessageListViewController * messageVC = [[BATMessageListViewController alloc] init];
        
        messageVC.view.frame = CGRectMake(0, 0, SCREEN_WIDTH, self.scrollView.frame.size.height);
...
#pragma mark - FooterViewDelegate && HeadViewDelegate
...
#pragma mark - 关注按钮数点击
- (void)focusCountButtonDidClik:(UIButton *)focusBtn{
...
- (BATSegmentControl *)segmentControl{
    if (!_segmentControl) {
        _segmentControl = [[BATSegmentControl alloc]initWithItems:@[@"消息",@"动态"]];
