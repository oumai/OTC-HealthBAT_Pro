...
- (void)requestWeChatPayFromBATAPI
{
    WEAK_SELF(self);
    [self showProgress];
    [HTTPTool requestWithOTCURLString:[NSString stringWithFormat:@"/api/pay/WxPay?orderNo=%@&payFee=%@",_orderNo,self.momey] parameters:nil type:kGET success:^(id responseObject) {
        STRONG_SELF(self);
...
- (void)requestAlipayFromBATAPI
{
    WEAK_SELF(self);
    [self showProgress];
    [HTTPTool requestWithOTCURLString:[NSString stringWithFormat:@"/api/pay/AliPay?orderNo=%@&payFee=%@",_orderNo,self.momey] parameters:nil type:kGET success:^(id responseObject) {
        STRONG_SELF(self);
        [self dismissProgress];
...
- (void)requestKMPayFromBATAPI
{
    ...
    [HTTPTool requestWithOTCURLString:[NSString stringWithFormat:@"/api/pay/KmPay?orderNo=%@&payFee=%@&returnUrl=https://www.kmpay518.com/",_orderNo,self.momey] parameters:nil type:kGET success:^(id responseObject) {
        STRONG_SELF(self);
        [self dismissProgress];
