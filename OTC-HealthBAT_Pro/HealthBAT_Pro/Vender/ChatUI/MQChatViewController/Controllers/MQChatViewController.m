...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handelWithMessage:) name:@"CUSTOM_MESSAGE_Room.StateChanged" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handelWithOrderBuyRecipeMessage:) name:@"CUSTOM_MESSAGE_Order.Buy.Recipe" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hangUp:) name:@"CUSTOM_MESSAGE_Room.Hangup" object:nil];
...
- (void)moveView:(UIPanGestureRecognizer *)pan{
    ...
    if (pan.view.center.x - batVideoView.frame.size.width/2.0 < 0) {
        [UIView animateWithDuration:0.25 animations:^{
            batVideoView.frame = CGRectMake(0, pan.view.center.y - batVideoView.frame.size.height/2.0, batVideoView.frame.size.width, batVideoView.frame.size.height);
        }];
    }
    
    if (pan.view.center.x + batVideoView.frame.size.width/2.0 > SCREEN_WIDTH) {
        [UIView animateWithDuration:0.25 animations:^{
            batVideoView.frame = CGRectMake(SCREEN_WIDTH - batVideoView.frame.size.width, pan.view.center.y - batVideoView.frame.size.height/2.0,batVideoView.frame.size.width, batVideoView.frame.size.height);
        }];
    }
    ...
    if (pan.view.center.y - batVideoView.frame.size.height/2.0 < 64) {
        [UIView animateWithDuration:0.25 animations:^{
            batVideoView.frame = CGRectMake(pan.view.center.x - batVideoView.frame.size.width/2.0, 64, batVideoView.frame.size.width, batVideoView.frame.size.height);
        }];
    }
}
...
- (void)keyboardWillShow:(NSNotification *)info{
    ...
    if (result && isVideoView == NO) {
        [UIView animateWithDuration:0.25 animations:^{
            batVideoView.frame = CGRectMake(batVideoView.frame.origin.x, 64, batVideoView.frame.size.width, batVideoView.frame.size.height);
        }];
    }
...
#pragma mark - private
- (void)hangUp:(NSNotification *)info {
...
//处理处方单消息
- (void)handelWithOrderBuyRecipeMessage:(NSNotification *)info {
    
    UIAlertController * alert = [UIAlertController alertControllerWithTitle:@"您收到了一条处方信息" message:@"是否结束咨询，查看处方单信息" preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction * cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        
        [self showText:@"处方单信息可在个人中心查看"];
    }];
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"去查看" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [self showProgressWithText:@"处理中，请稍后"];
        
        BATAppDelegate *appDelegate = (BATAppDelegate *)[UIApplication sharedApplication].delegate;
        BATRootTabBarController *rootVC = appDelegate.rootTabBarController;
        UINavigationController *currentNav = rootVC.viewControllers[rootVC.selectedIndex];
        [currentNav pushViewController:[[BATMyPrescriptionVC alloc] init] animated:NO];
